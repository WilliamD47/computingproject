    ...
    while (true) {
        ...
        if (choice == "Add") {
            ...
            if (addDone == false) {
                ...
                averages.push_back("For score 1:");
                averages.push_back(std::to_string(avg1));
                int sum2 = std::accumulate(std::begin(score2only), std::end(score2only), 0.0);
                int avg2 = sum2 / score2only.size();
                averages.push_back("For score 2:");
                averages.push_back(std::to_string(avg2));
                int sum3 = std::accumulate(std::begin(score3only), std::end(score3only), 0.0);
                int avg3 = sum3 / score1only.size();
                averages.push_back("For score 3:");
                averages.push_back(std::to_string(avg3));
                for (std::string item: averages)
                    std::cout << item << " \n";
            } else {
                std::cout << "You have not added a student yet!\n";
            }
